...
public class BinarySearchTree {
    ...
    public Song buscarPorTituloYArtista(String titulo, String artista) {
        return buscarPorTituloYArtistaRecursivo(raiz, titulo.trim().toLowerCase(), artista.trim().toLowerCase());
    }
    ...
    private Song buscarPorTituloYArtistaRecursivo(Node nodo, String titulo, String artista) {
        ...
        Song encontrado = buscarPorTituloYArtistaRecursivo(nodo.getIzquierdo(), titulo, artista);
        if (encontrado != null) {
            return encontrado;
        }
        
        // Verificar nodo actual
        if (nodo.getCancion().getTitulo().trim().toLowerCase().equals(titulo) &&
            nodo.getCancion().getArtista().trim().toLowerCase().equals(artista)) {
            return nodo.getCancion();
        }
        
        // Buscar en subárbol derecho
        return buscarPorTituloYArtistaRecursivo(nodo.getDerecho(), titulo, artista);
    }
    ...
    private int obtenerMaximoID(Node nodo) {
        if (nodo.getDerecho() == null) {
            return nodo.getCancion().getId();
        }
        return obtenerMaximoID(nodo.getDerecho());
    }
    
    // Buscar canciones por título (búsqueda parcial)
    public ArrayList<Song> buscarPorTitulo(String tituloBusqueda) {
        ArrayList<Song> resultados = new ArrayList<>();
        buscarPorTituloRecursivo(raiz, tituloBusqueda.trim().toLowerCase(), resultados);
        return resultados;
    }
    
    private void buscarPorTituloRecursivo(Node nodo, String tituloBusqueda, ArrayList<Song> resultados) {
        if (nodo != null) {
            buscarPorTituloRecursivo(nodo.getIzquierdo(), tituloBusqueda, resultados);
            
            if (nodo.getCancion().getTitulo().toLowerCase().contains(tituloBusqueda)) {
                resultados.add(nodo.getCancion());
            }
            
            buscarPorTituloRecursivo(nodo.getDerecho(), tituloBusqueda, resultados);
        }
    }
    
    // Buscar canciones por artista (búsqueda parcial)
    public ArrayList<Song> buscarPorArtista(String artistaBusqueda) {
        ArrayList<Song> resultados = new ArrayList<>();
        buscarPorArtistaRecursivo(raiz, artistaBusqueda.trim().toLowerCase(), resultados);
        return resultados;
    }
    
    private void buscarPorArtistaRecursivo(Node nodo, String artistaBusqueda, ArrayList<Song> resultados) {
        if (nodo != null) {
            buscarPorArtistaRecursivo(nodo.getIzquierdo(), artistaBusqueda, resultados);
            
            if (nodo.getCancion().getArtista().toLowerCase().contains(artistaBusqueda)) {
                resultados.add(nodo.getCancion());
            }
            
            buscarPorArtistaRecursivo(nodo.getDerecho(), artistaBusqueda, resultados);
        }
    }
